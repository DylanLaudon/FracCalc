package chapter05_section05;

import java.util.Scanner;

/**
   A program to compute shipping costs.
 */
public class Shipping
{
	public static void main(String[] args)
	{  
		Scanner in = new Scanner(System.in);

		System.out.println("Enter the state or province: ");
		String state = in.next();

		System.out.println("Enter the country: ");
		String country = in.next();
		in.close();

		double shippingCost;

		if (country.toUpperCase().equals("USA"))
		{
			if (state.toUpperCase().equals("AK") || state.toUpperCase().equals("HI")) // See Section 5.7 for the || (or) operator
			{
				shippingCost = 10;
			}
			else
			{
				shippingCost = 5;
			}
		}
		else
		{
			shippingCost = 10;
		}

		System.out.printf("Shipping cost to %s, %s: $%.2f\n",
				state, country, shippingCost);
	}
	
	public static double getShippingCost(String location)
	{  
		double shippingCost;
		
		Scanner in = new Scanner(location);
		String state = in.next();
		String country = in.next();
		
		//System.out.printf("State: %s, Country: %s", state, country);
		
		in.close();

		if (country.toUpperCase().equals("USA"))
		{
			if (state.toUpperCase().equals("AK") || state.toUpperCase().equals("HI")) // See Section 5.7 for the || (or) operator
			{
				shippingCost = 10;
			}
			else
			{
				shippingCost = 5;
			}
		}
		else
		{
			shippingCost = 10;
		}

		return shippingCost;
	}
	
	
}






